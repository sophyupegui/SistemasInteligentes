...
%% Inicialización de parametros 1 (originales):
...
%% Inicialización de parametros 2 (lb, ub):
...
% Límites de búsqueda para q0, q1, q2
lb = [0.5, 0.1, 0.01];   % Límites inferiores más altos para Kp y Ki
ub = [20, 10, 10];       % Límites superiores más amplios

%% Inicialización de parametros 3 (CR):

% N = 20;         % Tamaño de la población
% D = 3;          % Numero de parámetros (q0, q1, q2)
% CR = 0.4;       % Tasa de cruce (0 - 1)
% F_min = 0.9;    % Minimo valor de factor de escala (min: 0)
% F_max = 1.2;    % Maximo valor de factor de escala (max: 2)
% 
% % Límites de búsqueda para q0, q1, q2
...
%% Inicialización de parametros 4-1 (F):

% N = 20;         % Tamaño de la población
% D = 3;          % Numero de parámetros (q0, q1, q2)
% CR = 0.9;       % Tasa de cruce (0 - 1)
% F_min = 0.3;    % Minimo valor de factor de escala (min: 0)
% F_max = 0.7;    % Maximo valor de factor de escala (max: 2)
% 
% % Límites de búsqueda para q0, q1, q2
% lb = [0.1, 0.01, 0.01];
% ub = [10, 5, 5];

%% Inicialización de parametros 4-2 (F):

% N = 20;         % Tamaño de la población
% D = 3;          % Numero de parámetros (q0, q1, q2)
% CR = 0.9;       % Tasa de cruce (0 - 1)
% F_min = 1.5;    % Minimo valor de factor de escala (min: 0)
% F_max = 1.9;    % Maximo valor de factor de escala (max: 2)
% 
% % Límites de búsqueda para q0, q1, q2
% lb = [0.1, 0.01, 0.01];
% ub = [10, 5, 5];
...
hold on
title(['Respuesta del controlador PID inicial (Costo = ', num2str(first_fitness), ')'])
grid on
...
plot([ts_init ts_init], [0 1.2*max(y_init)], '--', Color='#A2142F')
plot([0 t_init(end)], [1+os_init/100 1+os_init/100], '--', Color='#D95319')
